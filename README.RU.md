...
- **Sequin WAL CDC (без прямого подключения к БД)**:
  ...
  - **3000 инстансов** (12 023 задач): Завершено за **46.34 с** при **64.74 RPS / 259.45 TPS** (оптимизированное управление конкурентностью, тюнинг G1GC, структурированное декодирование JSON и адаптивное управление пулом соединений. При пиковых нагрузках транзакционное давление на БД остается стабильным, завершено 100% задач без единого сбоя).
